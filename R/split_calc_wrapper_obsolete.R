...
#' #' @param buffer Extent each sub-cube* by this many cells (byrowcol=TRUE) or spatial units (byrowcol=FALSE)
#' #' @param fun The function to call per piece. Should return either raster or cube of identical dimensions as input for it.
#' #' @param ... passed to fun in argument list after the piece
#' #' @param dbg Print messages?
#' #' @param byrowcol Split by row-col indices instead of spatial extents? Default is TRUE
#' #'
#' #' @details
...
#' #' @import future stars progress
#' #' @importFrom methods is
#' #' @export
#' tj_split_calc_wrapper <- function(x, nx, ny, buffer = c(1,1),
#'                                   fun,
#'                                   ...,
...
#'
#'   wlist <- vector("list", length = nx * ny) # bounding boxes
#'   rclist <- vector("list", length = nx * ny) # row-col index tables
#'
#'   if(byrowcol){
