...
NumericMatrix local_stats(NumericMatrix x, int nrow,
  ...
  //if(neighbourhood == 1) forward_neighbour_cells = &forward_neighbour_cells_rook_row_col;
  //if(neighbourhood == 2) forward_neighbour_cells = &forward_neighbour_cells_queen_row_col;
  //
  for(i = 0; i < N; i++)
  if( !R_isnancpp( x(i,0) ) )  {
    out(i,0) += MKS(i);
...
/* Mann-Kendall S-statistic AND thiel-sen slope  */
// [[Rcpp::export]]
...
// we assume x[,i] is given per cell x-y order in "stars", like col-row in "raster".

// [[Rcpp::export]]
List c_contextual_mann_kendall(NumericMatrix x,
                               int nrow,
                               NumericVector time,
